...
public class TasksViewModel : ViewModel
{
    ...
    {
        bool isAllDeviceConnect = DeviceService != null && DeviceService.Devices != null &&
                                  !DeviceService.Devices.Any(d => d.IsConnected == false);
    ...
    {
        bool isClosedLastPallet = ReportTaskService.Statistic.PalletCodes.Count > 0 &&
                                  (ReportTaskService.Statistic.PalletCodes[^1].IsFulled ||
                                   ReportTaskService.Statistic.PalletCodes[^1].Boxes.Count == 0);
        return SelectedReportTask != null &&
               (SelectedReportTask.Status == "Запущено" ||
                SelectedReportTask.Status == "Остановлено") &&
               isClosedLastPallet;
    ...
    private void OnDeleteReportTaskCommandlCommandExecuted(object p)
    {
            ...
            SelectedReportTask.Status == "Сохранено")
        {
            ...
            ReportTaskService.ReportTasks = new List<ReportTask>(ReportTaskService.ReportTasks);
        }
        else
        {
    ...
    public TasksViewModel(ReportTaskService? reportTaskService,
        CreateTaskWindow createReportTaskWindow,
        IDeviceService deviceService)
    {
        ...
        createReportTaskCommand =
            new RelayCommand(OnCreateReportTaskCommandlCommandExecuted, CanCreateReportTaskCommandExecute);
        startReportTaskCommand =
            new RelayCommand(OnStartReportTaskCommandlCommandExecuted, CanStartReportTaskCommandExecute);
        pauseReportTaskCommand =
            new RelayCommand(OnPauseReportTaskCommandlCommandExecuted, CanPauseReportTaskCommandExecute);
        stopReportTaskCommand =
            new RelayCommand(OnStopReportTaskCommandlCommandExecuted, CanStopReportTaskCommandExecute);
        saveReportTaskCommand =
            new RelayCommand(OnSaveReportTaskCommandlCommandExecuted, CanSaveReportTaskCommandExecute);
        deleteReportTaskCommand =
            new RelayCommand(OnDeleteReportTaskCommandlCommandExecuted, CanDeleteReportTaskCommandExecute);
    }
}