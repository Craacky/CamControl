...
public class DeleteBoxAggregationViewModel : ViewModel
{
    private Pallet CurrentPallet { get; set; }
    ...
    public DeleteBoxAggregationViewModel(ProcessingCodeService processingCodeService,
        ...
        if (code.Contains("\0"))
        {
            ...
            if (isPalletCode)
            {
                ...
                if (isPalletCodeTheCurrentTask)
                {
                    ...
                    {
                        var pallets = LocalDBService.PalletDataService
                            .GetAllWithInclude(
                                p => p.MarkingCode.Replace("\u001d", "") == code.Replace("\u001d", "") &&
                                     p.ReportTaskGuid == ReportTaskService.CurrentReportTask.Guid, p => p.Boxes,
                                p => p.Products).ToList();
                        if (pallets.Count == 1)
                        {
            ...
            if (isBoxCode)
            {
                ...
                if (isBoxCodeTheCurrentTask)
                {
                    ...
                    if (isRepeatBoxCode)
                    {
                        ...
                        {
                            var deleteBox = boxes.FirstOrDefault(b => b.PalletId == CurrentPallet.Id);
                            ...
                            if (deleteBox != null)
                            {
                                ...
                                //Очищаем ошибки связанные с коробом
                                {
                                    var errors = ErrorsService.Errors.FindAll(e =>
                                        e.BoxCodes.Contains(deleteBox.MarkingCode));
                                ...
                                //Обновляем статистику
                                {
                                    ...
                                    ReportTaskService.Statistic.CountProducts =
                                        ReportTaskService.Statistic.ProductCodes.Count();
                        ...
                        else
                        {
                            var result = MessageBox.Show($"Найденно несколько коробов с данным кодом." +
                                                         $"\n{code}." +
                                                         $"Удалить последний найденный короб?", "",
                            ...
                            if (result == MessageBoxResult.Yes)
                            {
                                ...
                                var errors =
                                    ErrorsService.Errors.FindAll(e => e.BoxCodes.Contains(boxes[^1].MarkingCode));
                                foreach (var error in errors)
                                {
                                    ErrorsService.Errors.Remove(error);
                                }
                                ...
                                CurrentPallet.MarkingCode = CurrentPallet.MarkingCode.Split($"\u001d37")[0] +
                                                            $"\u001d37" +
                                                            $"{Convert.ToInt32(CurrentPallet.Boxes.Count) * Convert.ToInt32(ReportTaskService.CurrentReportTask.CountProductInBox)}";
                                CurrentPallet =
                                    LocalDBService.PalletDataService.Update(CurrentPallet.Id, CurrentPallet);
                                ...
                                ReportTaskService.Statistic.BoxCodes =
                                    new List<Box>(ReportTaskService.Statistic.BoxCodes);
                                ...
                                ReportTaskService.Statistic.ProductCodes =
                                    new List<Product>(ReportTaskService.Statistic.ProductCodes);
